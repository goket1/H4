            ...
            using (var con = new NpgsqlConnection(cs))
            {
                ...
                switch (Menu("Select\n1. Create user\n2. Login", 2))
                {
                    case 1:
                        ...
                        byte[] hashed = hash.ComputePasswordHash(Encoding.UTF8.GetBytes(password), salt);
                    ...
                    case 2:
                        ...
                        else
                        {
                            ...
                            byte[] hashFromUser = hash.ComputePasswordHash(Encoding.UTF8.GetBytes(password), db_salt);
                            ...
                            else
                            {
                                ...
                                using (var cmd = new NpgsqlCommand("CALL wrong_password(@username, @attempts)", con))
                                {
                                    cmd.Parameters.AddWithValue("username", username);
                                    cmd.Parameters.AddWithValue("attempts", db_attempts + 1);
                                    ;
                                    cmd.ExecuteNonQuery();
                                }
                            }
